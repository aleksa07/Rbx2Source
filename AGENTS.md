...
- **R6 animation space** — R6 `KeyframeSequence` poses are delta offsets in Roblox's animation space. `AnimationBuilder.Assemble` converts them with hand-crafted axis patches (Torso YZ-flip, right-side X-invert, ±90° arm/leg position swap, rotation inverse for non-Head) and writes them WITHOUT `DeltaSequence` — R6 frames stay raw and the QC marks R6 sequences `$delta` (the SMD values are authored deltas relative to the rig ReferencePos). R15 poses are local deltas (plus an Euler-angle reorder) and DO use `DeltaSequence` with `BaseRig`. Do NOT route R6 through `DeltaSequence`: composing the raw pose with `refBone.C0` turns the limb swing into a pivot translation (limbs float instead of swinging).
