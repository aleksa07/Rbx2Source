...
- **Layered clothing** — explicitly unsupported per README; `LayeredClothingExtractor` exists but is partial/incomplete. Hardened: plugin dir created via `Directory.CreateDirectory`, registry keys null-guarded (`?.` + `using`), Studio launch and OBJ export have timeouts (60s / 5min), process kill null/exit-guarded, empty/whitespace OBJ skipped, `ObjFile` parse wrapped in try-catch. Errors in layered clothing flow surface through the recursive `GetRootException` unwrap.
