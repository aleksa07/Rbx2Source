...
- **R6 animation space** — R6 `KeyframeSequence` poses are local delta offsets in the classic R6 Motor6D frame. `CharacterBase.rbxm` rig attachments deliberately use those exact joint frames (shoulder/hip `Ry(±90°)`, neck YZ-swapped), so `AnimationBuilder.Assemble` writes the raw interpolated delta and lets the `DeltaSequence` writer compose it with `refBone.C0` — same mechanism as R15, no hand-crafted axis patches. The QC marks R6 sequences `$delta` (absolute SMD values; deltas are authored directly). If the delta is written raw without `DeltaSequence`, large-motion animations (Run/Climb/Jump) scramble the avatar in-game. R15 poses are also local deltas (plus an Euler-angle reorder) and use `DeltaSequence` with `BaseRig` — R6 and R15 differ only in that reorder.
