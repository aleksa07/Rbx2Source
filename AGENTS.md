...
- **R6 animation space** — R6 `KeyframeSequence` poses are delta offsets in Roblox's animation space. `AnimationBuilder.Assemble` must translate them into Source skeleton space before writing the SMD (Torso YZ axis flip, right-side X inversion, arms/legs 90° CCW position rotation, rotation inverse for non-Head), and the QC marks R6 sequences `$delta` (absolute SMD values; deltas are authored directly). If these patches are missing, large-motion animations (Run/Climb/Jump) scramble the avatar in-game while the reference pose still looks correct. R15 poses are absolute world CFrames and use a `DeltaSequence` with `BaseRig` instead — do not apply R6 patches to R15.
