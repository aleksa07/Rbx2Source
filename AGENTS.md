...
- **R6 animation space** — R6 `KeyframeSequence` poses are raw Roblox deltas whose joint frames match the classic R6 Motor6D attachments (shoulder/hip `Ry(±90)`, neck YZ-swap). `AnimationBuilder.Assemble` routes R6 through `DeltaSequence` with `BaseRig` (same as R15), so the writer emits absolute frames `refBone.C0 * pose` (exactly Roblox's `boneCFrame = C0 * pose`), and the QC marks R6 sequences WITHOUT `$delta` (same as R15). The hand-crafted axis patches (Torso YZ-flip, right-side X-invert, ±90° arm/leg position swap, rotation inverse) were removed — they mirrored the exported motion (right arm swung forward at t=0 instead of backward). Do NOT add `$delta` back to R6 sequences: the frames are already absolute. R15 poses additionally get an Euler-angle reorder before the same `DeltaSequence` write path.
